%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% This code computes wavelet domain features for case subjects 
%  (1) Level-wise cross correlation
%  (2) Wavelet entropy
%  (3) Spectral slope 

%This code computes time domain features for case subjects  
...
dirName = sprintf('Data/Case/');             %# folder path
...
for i = 1 : nfi
    ...
    if size(data,1) >= n
        ...
        for p = 2:9
            ...
            lf1 = wc_lf( round(2^(J-L1)+1) : end ); rf1 = wc_rf( round(2^(J-L1)+1) : end );
...
for i = 1 : nfi
    ...
    if size(data,1) >= n
        ...
        for p = 2:9
            ...
            %rf - left foot
            rf = diff(zscore(data(1:n+1, p+8)), 1); wc_rf = dwtr(rf, filt, J - L2);
...
for i = 1 : nfi
    ...
    if size(data,1) >= n
       ...
       Case_HeelToe  = [Case_HeelToe ; [ mean(VGRF_hr(:,1)) std(VGRF_hr(:,1)) ...
...
for i = 1 : nfi
    ...
    if size(data,1) >= n
        ...
        Case_St = [ Case_St; [ mean(sw_L)  mean(st_L)] ];
    end 
end 

Cse.Case_CV = Case_CV;
Cse.Case_Ent = Case_As;
Cse.Case_Slp = Case_Slope;
Cse.Case_St = Case_St;
Cse.Case_HeelToe = Case_HeelToe;

save('CaseFeatures/Cse.mat','Cse')
